package exercises.struct_conc.sc_02_sub_scopes.base;
...
public class SubScope_BASE {
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException {
		...
		SubScope_BASE instance = new SubScope_BASE();
	...
	void structuredConcurrencyNestedScope_A() throws InterruptedException, ExecutionException {
		...
		// TODO: Scope oeffnen, dies ist der aussere, der umschliessende Scope. //try (var outerScope = StructuredTaskScope.open()) { /... }
			System.out.println("Outer-Scope opened");
			
			// TODO Subscope Variante 1: innerhalb eines Scopes try (var subScopeTopLevel = StructuredTaskScope.open()) { //... }
				System.out.println("Sub-Scope within Top-Level Scope opened (Variant 1)");
				
				// TODO Subtask starten, im Nested-Scope Variante 1 //subScopeTopLevel.fork(() -> { });
				
					// Beispiel-Code der in der Task lauf koennte
					System.out.println("Task 1 started in Sub-Scope (Variant 1)");
					Thread.sleep(4000);
					System.out.println("Task 1 returning");
							
				// TODO Subscope abwarten // subScopeTopLevel.join();
				System.out.println("Sub-Scope Variant 1 join returned at Second " + ((System.currentTimeMillis() - starttime) / 1000));

			

			// TODO: Auf Beendigung aller Subscopes warten // outerScope.join();
 ...
 *  	Subscope. Darin startest Du dann wiederum einen Task-3. Das ist dann die Variante 2.
	...
	void structuredConcurrencyNestedScope_B() throws InterruptedException, ExecutionException {
		...
		////////////// AUFGABENTEIL A: Hier schon fertig programmiert ///////////////////
		...
		try (var outerScope = StructuredTaskScope.open()) {
			...
			// TODO Subtask im aeusseren Scope starten outerScope.fork(() -> { //... });

				System.out.println("Task 2 started: In Top-Level Scope");

				// TODO Sub-Scope Variante 2: innerhalb eines Tasks // try (var subScopeWithinSubtask = StructuredTaskScope.open()) { //... }
					System.out.println("Sub-Scope within Task opened (Variant 2)");
					// TODO Subtask 3 starten //subScopeWithinSubtask.fork(() -> { //... });
						// Beispielcode innerhalb Task-3
						/*
					...
					// TODO auf innere Subtask warten: // subScopeWithinSubtask.join();
